<!-- Main Sidebar Container -->
<aside class="main-sidebar sidebar-dark-primary elevation-4">
  <!-- Brand Logo -->
  <a href="index.php?page=admin" class="brand-link">
    <img src="../../dist/img/AdminLTELogo.png" alt="AdminLTE Logo" class="brand-image img-circle elevation-3"
    style="opacity: .8">
    <span class="brand-text font-weight-light"><?php echo $_SESSION['type'];?></span>
  </a>

  <!-- Sidebar -->
  <div class="sidebar">
    <!-- Sidebar user panel (optional) -->
    <div class="user-panel mt-3 pb-3 mb-3 d-flex">
      <div class="image">
        <img src="../../dist/img/avatar5.png" class="img-circle elevation-2" alt="User Image">
      </div>
      <div class="info">
        <a href="index.php?page=admin" class="d-block"><?php echo $_SESSION['nama'];?></a>
      </div>
    </div>

    <!-- Sidebar Menu -->
    <nav class="mt-2">
      <ul class="nav nav-pills nav-sidebar flex-column" data-widget="treeview" role="menu" data-accordion="false">
        <!-- Add icons to the links using the .nav-icon class
         with font-awesome or any other icon font library -->
         <li class="nav-item">
          <a href="index.php?page=admin" class="nav-link">
            <i class="nav-icon fas fa-chart-line"></i>
            <p>Dashboard</p>
          </a>
        </li>
        <li class="nav-item">
          <a href="index.php?page=account" class="nav-link">
            <i class="nav-icon fas fa-address-book"></i>
            <p>Account</p>
          </a>
        </li>
        <li class="nav-item">
          <a href="index.php?page=pemesanan" class="nav-link">
            <i class="nav-icon fas fa-shopping-cart"></i>
            <p>Pemesanan</p>
          </a>
        </li>
        <li class="nav-item">
          <a href="index.php?page=kereta" class="nav-link">
            <i class="nav-icon fas fa-train"></i>
            <p>
              Kereta
            </p>
          </a>
        </li>
        <li class="nav-item">
          <a href="index.php?page=jadwal" class="nav-link">
            <i class="nav-icon far fa-calendar-alt"></i>
            <p>
              Jadwal
            </p>
          </a>
        </li>
        <li class="nav-item">
          <a href="index.php?page=stasiuna" class="nav-link">
            <i class="nav-icon fas fa-hourglass-start"></i>
            <p>
              Stasiun Awal
            </p>
          </a>
        </li>
        <li class="nav-item">
          <a href="index.php?page=stasiunb" class="nav-link">
            <i class="nav-icon fas fa-hourglass-end"></i>
            <p>
              Stasiun Akhir
            </p>
          </a>
        </li>
      </ul>
    </nav>
    <!-- /.sidebar-menu -->
  </div>
  <!-- /.sidebar -->
</aside>
